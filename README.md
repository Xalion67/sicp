...
>Трансляторы пайтона, с++ и скиима работаю с апликативным порядком


## [Задание 1.6](#)  
В этом задании объясняют почему `if` должна быть особой формой, а не обычной функцией написанной через `cond`. `cond` ‒ это условная конструкция в скиме для вычисления нескольких условий.  
...
> Это часть кода для вычисления квадратного корня, [полный код в задании 1.7](/chapter_1/1.7/sqrt.py)
```scm
...
**Решение**  
Интерпретатор упадёт от бесконечной рекурсии, потому что работает с аппликативным порядком вычислений.  
...
## [Задание 1.7](/chapter_1/1.7/sqrt.py)
Функция для вычисления корня объясняет смысл процедурной абстракции как черный ящик и блочную структуру.
Первый вариант функции выглядит вот так
```py
def sqrt(x):
...
Но здесь есть недостатки: она вычисляет неправильный квадратный корень для слишком маленьких и больших чисел, ещё все дополнительные функции написаны снаружи. 
